...
CLASSIFIER_FILE = dir_path + '/' +'haarcascade_frontalface_default.xml'
PICKLE_FILE = dir_path + '/' +'encodings.pickle'
CONFIG_NAMES_FILE = dir_path + '/' +'names.config'
...
while True:
	...
	for encoding in encodings:
		...
		if True in matches:
			...
			max_votes = max(counts.iteritems(), key=operator.itemgetter(1))[1]
	...
	if name != "Unknown" and activeNames.get(name) and max_votes > min_vote_threshold:
                print 'detected ', name
		speechFilename=name + ".mp3"
		os.system("mpg321 -q " + dir_path +"/audio/" + speechFilename)
